...
namespace NAnt.Core {
    ...
    class DirectoryScanner : ICloneable {
        ...
        /// </remarks>
        //ported: changed PlatformHelper.IsUnix to false
        public DirectoryScanner () : this (/*PlatformHelper.IsUnix*/ false) {
        }
        ...
        private void ConvertPatterns(StringCollection nantPatterns, ArrayList regexPatterns, StringCollection nonRegexFiles, bool addSearchDirectories) {
            ...
            foreach (string nantPattern in nantPatterns) {
                ...
                if (isRegex) {
                   ...
                   //ported: commented next two lines
        ...
        private void ParseSearchDirectoryAndPattern(bool isInclude, string originalNAntPattern, out string searchDirectory, out bool recursive, out bool isRegex, out string regexPattern) {
           ...
           //ported: changed next line to following introduced one
            ...
            if (searchDirectory.EndsWith("" + Path.DirectorySeparatorChar) && (searchDirectory.Length != 3 || searchDirectory[1] != Path.VolumeSeparatorChar))
            {

...
{
   public class FloatRangeExtended : IEquatable<FloatRangeExtended>
   {
      /// <summary>
      ...
      private FloatRangeExtended(FloatBehaviour floatBehaviour, NuGetVersion minVersion, string releasePrefix, string originalString)
      {
      ...
      /// </summary>
      public static FloatRangeExtended Parse(string versionString)
      {
      ...
      /// Parse a floating version into a FloatRange
      /// </summary>
      public static bool TryParse(string versionString, out FloatRangeExtended range)
      {
         range = null;
         ...
         var isPrereleaseOnly = hasReleaseStar && (releaseVersion.Length == 1 || releaseVersion.Length > 1 && releaseVersion[releaseVersion.Length - 2] != '.');
         ...
         if (numberVersion.IndexOf('*', 0, numberVersion.Length - 1) != -1 || (releaseVersion?.IndexOf('*', 0, releaseVersion.Length - 1) ?? -1) != -1)
            return false;
      ...
      public override string ToString()
      {
         ...
         if (FloatBehaviour == FloatBehaviour.RevisionPrerelease)
            return string.Format(CultureInfo.InvariantCulture, "{0}.{1}.{2}-{3}*", MinVersion.Major, MinVersion.Minor, MinVersion.Patch, OriginalReleasePrefix);
      ...
      public bool Equals(FloatRangeExtended other)
      {
      ...
      private static bool SetRangeVersion(string versionString, FloatBehaviour behaviour, string release, out FloatRangeExtended floatVersion, string originalString)
      {
         floatVersion = NuGetVersion.TryParse(versionString, out var version)
            ? new FloatRangeExtended(behaviour, version, release, originalString)
            : null;
      ...
      private bool CompareRelease(NuGetVersion version, FloatBehaviour floatBehaviour)
      {
